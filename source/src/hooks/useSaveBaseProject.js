...
const useSaveBaseProject = ({
    ...
    const { execute: executeGet, loading } = useFetch({ ...apiConfig.getById,authorization: `Bearer ${userTokenProject}` }, {
        immediate: false,
    });
    const translate = useTranslate();
  
    const { execute: executeCreate, loading: loadingCreate } = useFetch({ ...apiConfig.create,authorization: `Bearer ${userTokenProject}` }, { immediate: false });
    const { execute: executeUpdate, loading: loadingUpdate } = useFetch({ ...apiConfig.update,authorization: `Bearer ${userTokenProject}` }, { immediate: false });
    const title =
        params.id === 'create'
...
export default useSaveBaseProject;
