...
const useListBaseTab = ({
    ...
    const prepareGetListParams = (filter) => {
        const copyFilter = { ...filter, ...options.queryPage };
    ...
    const getList = (filter) => {
        const params = mixinFuncs.prepareGetListParams({ ...options.queryPage, ...filter });
